...
def predict(X_img_path, clf=None, model_path=None, distance_threshold=0.6):
    ...
    if model_path == "trained_knn_model.clf" :
        closest_distances = clf.kneighbors(faces_encodings, n_neighbors=3,  return_distance=True)
        print closest_distances
        
        are_matches = [closest_distances[0][i][0] <= distance_threshold for i in range(len(faces_encodings))]
        print are_matches
        result = clf.predict(faces_encodings)
        print "knn predict result:"
        print (result)
        # Predict classes and remove classifications that aren't within the threshold
        return [(pred) if rec else ("unknown") for pred, rec in zip(clf.predict(faces_encodings), are_matches)]
    ...
    while True:
        ...
        if process_this_frame:
            ...
            if len(face_locations) > 0 :
                img_path_x = "test/image" +str(datetime.datetime.now())
                img_path = img_path_x + ".jpg"
                print("found face : " +img_path )
                cv2.imwrite(img_path, rgb_small_frame)

            #put to predict
            	knn_predictions = predict(img_path, model_path="trained_knn_model.clf")
            	print knn_predictions
            	svm_predictions = predict(img_path, model_path="trained_svm_model.clf")
            	print svm_predictions
            	nb_predictions = predict(img_path, model_path="trained_bayes_model.clf")
            	print nb_predictions
		    #outputs file directory with predicted data
			
			print "knn:"
			show_prediction(img_path, knn_predictions)
			#print "svm:"
